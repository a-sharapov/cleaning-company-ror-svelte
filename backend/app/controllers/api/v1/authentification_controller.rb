class Api::V1::AuthentificationController < ApplicationController
  ...
  def refresh
    begin
      refresh_token = get_refresh_token
      escape_with!(:auth, :unauthorized_access, :unauthorized, "unauthorize") unless refresh_token_data = AuthentificationTokenService.decode_token(refresh_token)
      user = User.find_by(login: refresh_token_data.first["login"])
      escape_with!(:user, :not_exist, :ok) unless user 
      escape_with!(:auth, :unauthorized_access, :unauthorized, "unauthorize") unless user.tokens.any? && user.tokens.find(refresh_token).any?
      response = {
        message: ApiError::MESSAGES[:auth][:update], 
        user: except_data!(user).merge(
          access_token: AuthentificationTokenService.reissue_access_token(user, @metrics)
        )
      }
      render json: response, status: :ok
    rescue ApiError => e
  ...
  def remove
    begin
      ...
      render json: {message: "Сессия была удалена"}, status: :ok
    rescue ApiError => e
      render_api_error(e)
    end
  end
